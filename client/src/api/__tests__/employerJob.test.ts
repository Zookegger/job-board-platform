...
describe("employerJobApi", () => {
  ...
  it("getEmployerJobs ném ApiError khi thất bại", async () => {
    ...
    await expect(employerJobApi.getEmployerJobs({}))
      .rejects.toThrow(/Không thể tải danh sách/)
  })
  ...
  it("getEmployerJobDetail ném ApiError với status 404", async () => {
    ...
    await expect(employerJobApi.getEmployerJobDetail("not-found"))
  ...
  it("createEmployerJob ném ApiError khi thất bại", async () => {
    ...
    await expect(employerJobApi.createEmployerJob({
      title: "Bad Job",
      description: "x",
      categoryId: 1,
      locationTypes: "ONSITE",
  ...
  it("submitForReview ném ApiError khi thất bại", async () => {
    ...
    await expect(employerJobApi.submitForReview(mockJobResponse.id))
      .rejects.toThrow(/Gửi duyệt thất bại/)
  })
  ...
  it("getCategories trả về danh sách ngành nghề", async () => {
    ...
    const result = await employerJobApi.getCategories()
  ...
  it("getCategories ném ApiError khi thất bại", async () => {
    ...
    await expect(employerJobApi.getCategories())
      .rejects.toThrow(/Không thể tải danh sách ngành nghề/)
  })
})
